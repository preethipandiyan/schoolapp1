...
export default function LeaveRequests() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let docInfo = null;
      if (selectedFile) {
        const storagePath = `schools/${schoolId}/leaves/${currentUser.uid}_${Date.now()}_${selectedFile.name}`;
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <div className="mb-8 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Leave Requests</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Apply for leaves and track your approval status.</p>
        </div>
        <button 
